...
class LearnerProfile:
    ...
    def update_profile(self, student_id: str, quiz_answers: Dict[str, Dict]) -> Dict[str, float]:
        """Update learner profile based on quiz answers"""
        if not quiz_answers:
            return self.get_profile(student_id) or {'accuracy': 0.0, 'pace': 0.0, 'engagement': 0.0}
        
        # Calculate current session metrics
        total_questions = len(quiz_answers)
        correct_answers = sum(1 for answer in quiz_answers.values() if answer.get('correct', False))
        skipped_questions = sum(1 for answer in quiz_answers.values() if answer.get('skipped', False))
        response_times = [answer.get('response_time', 0) for answer in quiz_answers.values()]
        
        # Current session metrics
        current_accuracy = correct_answers / total_questions if total_questions > 0 else 0
        current_pace = sum(response_times) / len(response_times) if response_times else 0
        current_engagement = 1 - (skipped_questions / total_questions) if total_questions > 0 else 0
        ...
        # Calculate weighted averages (more weight to recent performance)
        quiz_count = existing_profile.get('quiz_count', 0)
        weight = 0.7 if quiz_count > 0 else 1.0  # Give more weight to current session
        
        # Update metrics with weighted average
        new_accuracy = (existing_profile.get('accuracy', 0) * (1 - weight) + 
                       current_accuracy * weight)
        new_pace = (existing_profile.get('pace', 0) * (1 - weight) + 
                   current_pace * weight)
        new_engagement = (existing_profile.get('engagement', 0) * (1 - weight) + 
